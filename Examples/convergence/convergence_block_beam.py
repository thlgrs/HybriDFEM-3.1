...
# CONFIGURATION
# =============================================================================
RESULTS_ROOT = str(project_root / "Examples" / 'Results' / 'Block' / 'convergence_beam')

BASE_CONFIG = {
    'geometry': {
        'height': 0.5,
        'thickness': 0.2,
        'n_slices': 1,  # Fixed: 4 slices (block-fem-block-fem)
        'start_with': 'block',
        'block_slice_width': 3,
        'fem_slice_width': 0,
    ...
    for elem_key in element_types.keys():
        ...
        for i, nx in enumerate(nx_values):
            ...
            ax.plot(df_nx['ny'], df_nx[f'disp_{elem_key}'],
                    ...
                    color=c, alpha=0.3 + 0.7 * i / len(nx_values), label=label)

    ax.set_xlabel(r"Vertical refinement $n_y$", fontsize=12)
    ax.set_ylabel(r"Tip Displacement $u_y$ [m]", fontsize=12)
    ...
    for elem_key in element_types.keys():
        ...
        for i, ny in enumerate(ny_values):
            ...
            ax.plot(df_ny['nx'], df_ny[f'disp_{elem_key}'],
                    marker='o', markersize=4, linestyle='-', linewidth=1.5,
                    color=c, alpha=0.3 + 0.7 * i / len(ny_values), label=label)
...
NX_RANGE = np.flip(3 * np.unique(np.geomspace(2, 40, num=20, dtype=int)))
