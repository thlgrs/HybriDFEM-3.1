...
RESULTS_ROOT = str(project_root / "Examples" / 'Results' / 'FEM' / 'fem_square')
from fem_square import BASE_CONFIG, apply_conditions
...
def tip_displacement(config):
    ...
    St = run_solver(St, config)
    if tip_nodes:
        dof_y = St.get_dofs_from_node(tip_nodes)[1]
        return St.U[dof_y]
    return np.nan
...
NX_RANGE = np.unique(np.geomspace(1, 100, num=2, dtype=int))
NY_RANGE = np.unique(np.geomspace(1, 16, num=2, dtype=int))
