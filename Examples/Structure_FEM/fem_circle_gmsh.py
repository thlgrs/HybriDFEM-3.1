...
from Examples.utils import plot_deformed, plot_initial, plot_displacement
...
RESULTS_ROOT = str(project_root / "Examples" / 'Results' / 'FEM' / 'illustration')
...
def generate_fem_mesh_gmsh(config):
    ...
    x_coords = width / 2 + r * np.cos(angles)
    y_coords = height / 2 + r * np.sin(angles)
    points = [(x, y) for x, y in zip(x_coords[1:], y_coords[1:])]
    forme_1 = [
        (0.0, 0.0),  # Bas-Gauche
        (0.4, 0.0),  # Bas-Droit (plus large en bas)
        (0.25, 1.0),  # Haut-Droit (plus étroit en haut)
        (0.0, 1.0)  # Haut-Gauche
    ]
    # Départ haut gauche, sens anti-horaire
    forme_2 = [
        (0.0, 1.0 / 4),  # Haut-Gauche
        (0.0, 0.5 / 4),  # Début épaisseur dalle gauche
        (0.05, 0.0 / 4),  # Bas nervure 1 (gauche)
        (0.25, 0.0 / 4),  # Bas nervure 1 (droite)
        (0.30, 0.5 / 4),  # Remontée nervure 1
        (0.35, 0.5 / 4),  # Plat intermédiaire
        (0.40, 0.0 / 4),  # Bas nervure 2 (gauche)
        (0.60, 0.0 / 4),  # Bas nervure 2 (droite)
        (0.65, 0.5 / 4),  # Remontée nervure 2
        (0.70, 0.5 / 4),  # Plat intermédiaire
        (0.75, 0.0 / 4),  # Bas nervure 3 (gauche)
        (0.95, 0.0 / 4),  # Bas nervure 3 (droite)
        (1.0, 0.5 / 4),  # Remontée finale droite
        (1.0, 1.0 / 4)  # Haut-Droit
    ]
    # Départ coin inférieur gauche, sens anti-horaire
    forme_3 = [
        (0.0, 0.0),  # Bas-Gauche aile inférieure
        (1.0, 0.0),  # Bas-Droit aile inférieure
        (1.0, 0.15),  # Haut-Droit aile inférieure
        (0.55, 0.15),  # Coin intérieur droit bas (jonction âme)
        (0.55, 0.85),  # Coin intérieur droit haut (jonction âme)
        (1.0, 0.85),  # Bas-Droit aile supérieure
        (1.0, 1.0),  # Haut-Droit aile supérieure
        (0.0, 1.0),  # Haut-Gauche aile supérieure
        (0.0, 0.85),  # Bas-Gauche aile supérieure
        (0.45, 0.85),  # Coin intérieur gauche haut (jonction âme)
        (0.45, 0.15),  # Coin intérieur gauche bas (jonction âme)
        (0.0, 0.15)  # Haut-Gauche aile inférieure
    ]
    # Départ coin inférieur gauche (extérieur), sens anti-horaire
    forme_4 = [
        (0.0, 0.0),  # Coin extérieur bas
        (1.0, 0.0),  # Pointe droite bas
        (1.0, 0.25),  # Épaisseur jambe horizontale
        (0.25, 0.25),  # Coin intérieur
        (0.25, 1.0),  # Pointe haut intérieur
        (0.0, 1.0)  # Pointe haut extérieur
    ]
...
def apply_conditions(St, config):
    ...
    F = config['loads']['Fy']
    tol = 1e-6
    search_tol = config['elements'].get('size') / 2
    b_set = set(find_nodes_at_height(St, 0, tolerance=search_tol))
    l_set = set(find_nodes_at_length(St, 0, tolerance=search_tol))
    r_set = set(find_nodes_at_length(St, L, tolerance=search_tol))
    t_set = set(find_nodes_at_height(St, H, tolerance=search_tol))

    St.fix_node(list(b_set), [1])
    St.load_node(list(t_set), dofs=[1], force=F / len(list(t_set)))
    return St, list(t_set)[0]
...
def analyze_results(St, config, control_node):
    ...
    lines.append(f"- **Geometry**: W = {g_conf['width']} m, H = {g_conf['height']} m, t = {g_conf['thickness']} m")

    bc_type = l_conf.get('name', 'compression')
    if bc_type == 'shear':
        lines.append(f"- **Loading**: Fx = {l_conf.get('Fx', 0) / 1000:.1f} kN (shear)")
    else:
        lines.append(f"- **Loading**: Fy = {l_conf.get('Fy', 0) / 1000:.1f} kN (compression)")
...
COMPRESSION_CONFIGS = element_configs('compression', [0.05])
