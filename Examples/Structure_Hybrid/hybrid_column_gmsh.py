...
# CONFIGURATION
# =============================================================================
RESULTS_ROOT = str(project_root / 'Examples' / 'Results' / 'Hybrid' / 'hybrid_column_gmsh')

BASE_CONFIG = {
    'geometry': {
        'width': 1.0,  # Column width (m)
        'thickness': 1.0,  # Out-of-plane thickness (m)
        'n_slices': 40,  # Number of alternating slices
        'start_with': 'block',  # First slice type at bottom
        'block_slice_height': 0.1,  # Height of block slices (m)
        'fem_slice_height': 0.1,  # Height of FEM slices (m)

        # Mesh Refinement
        'nx': 20,  # Blocks per row (horizontal)
    ...
    'contact': {
        'kn': 200e9,  # Normal contact stiffness (N/m)
        'ks': 200e9,  # Shear contact stiffness (N/m)
        'LG': True,  # Linear geometry assumption
    ...
    'loads': {
        'Type': 'Point',
        'Fx': 5e5,  # Horizontal force at top (N)
        'Fy': -5e5,  # Vertical force at top (N)
    },
    'bc': {
        'type': 'cantilever',  # Fix base
    },
    'io': {
        'filename': 'hybrid_column_gmsh',
        'dir': RESULTS_ROOT,
        'show_nodes': False,
        'scale': 50,
        'figsize': (4, 7),
...
def create_model(config, use_batch_generation=True):
    ...
    for i in range(n_slices):
        ...
        if is_block:
            ...
            St_fem = Structure_FEM.from_mesh(mesh, mat_fem, geom)
...
def apply_conditions(St, config):
    """Apply cantilever boundary conditions: fixed base, load at top."""
    g = config['geometry']
    l_conf = config['loads']
    ...
    center_x_set = set(find_nodes_at_length(St, total_width / 2, tolerance=5e-2))

    if bc_type == 'cantilever':
        # Fix base (bottom edge) fully
        nodes_to_fix = list(bottom_set)
        for node in nodes_to_fix:
            St.fix_node(node_ids=[node], dofs=[0, 1, 2])
        print(f"  -> Fixed Base ({len(nodes_to_fix)} nodes)")

        # Load applied at top
        nodes_to_load = list(top_set)
        control_node = list(top_set.intersection(center_x_set))
        if control_node:
    ...
    print(f"  -> Applied load at {len(nodes_to_load)} top nodes")
    print(f"  -> Fx = {l_conf.get('Fx', 0) / 1e3:.1f} kN, Fy = {l_conf.get('Fy', 0) / 1e3:.1f} kN")
...
MORTAR_T3 = create_config(BASE_CONFIG, 'Hybrid_Column_Mortar_T3_gmsh', elements={'type': 'triangle', 'order': 1},
                          coupling={'method': 'mortar'})
MORTAR_T6 = create_config(BASE_CONFIG, 'Hybrid_Column_Mortar_T6_gmsh', elements={'type': 'triangle', 'order': 2},
                          coupling={'method': 'mortar', 'integration_order': 3})
MORTAR_Q4 = create_config(BASE_CONFIG, 'Hybrid_Column_Mortar_Q4_gmsh', elements={'type': 'quad', 'order': 1},
                          coupling={'method': 'mortar'})
MORTAR_Q9 = create_config(BASE_CONFIG, 'Hybrid_Column_Mortar_Q9_gmsh', elements={'type': 'quad', 'order': 2},
                          coupling={'method': 'mortar', 'integration_order': 3})

ALL_ELEMENTS = [MORTAR_T3, MORTAR_T6, MORTAR_Q4, MORTAR_Q9]
# =============================================================================
# MAIN
# =============================================================================

if __name__ == "__main__":
    run_configs(ALL_ELEMENTS)
