...
def plot_element_comparison(filename, fixed_param, metric='displacement', target=None):
    ...
    fixed_val = sorted(df[fixed_param].unique())[3]
...
if __name__ == "__main__":
    ...
    for element in ELEMENTS:
        ...
        plot_sensitivity(FILENAME, element, 'nx', metric='error', target=timoshenko)
        plot_sensitivity(FILENAME, element, 'ny', metric='error', target=timoshenko)
    ...
    plot_element_comparison(FILENAME, 'nx', metric='error', target=timoshenko)
    plot_element_comparison(FILENAME, 'ny', metric='error', target=timoshenko)