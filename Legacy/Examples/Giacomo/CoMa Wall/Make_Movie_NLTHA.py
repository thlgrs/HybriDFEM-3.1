...
for file_name in os.listdir():
    ...
    if file_name.endswith('0.25.h5') and file_name.startswith(f'CoMa_{pga}'):
...
Horiz = np.zeros(len(U_conv[:, 0]))
for i in range(len(U_conv[:, 0])):
    if i % 3 == 0:
        Horiz[i] = 1
...
for i in range(frames_needed):
    U_rigid = U_conv[14 * 3, i * ratio]
    St.U = U_conv[:, i * ratio] - U_rigid * Horiz
    # St.U = U_conv[:,i*ratio]
    saveto = save_path + f'/Frame{i:0{power}d}.png'
    St.plot_structure(scale=5, plot_cf=False, plot_forces=False, show=False, save=saveto,
                      lims=[[0.5, 1.7], [4.5, 6.75]])
    # St.plot_structure(scale=1, plot_cf=False, plot_forces=False, show=False, save=saveto, lims=[[-1.,7], [-1, 6.75]])
...
video_filename = f'Video_CoMa_{pga}_Full.mp4'
